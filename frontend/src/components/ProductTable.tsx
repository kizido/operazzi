...
export default function ProductTable() {
  ...
  const columns: ColumnDef<ProductModel>[] = [
    ...
    },
    {
      ...
      cell: (info) => {
        ...
        if (
          dimensions &&
          dimensions.productLength &&
          dimensions.productWidth &&
          dimensions.productHeight
        ) {
  ...
  async function deleteProduct(product: ProductModel) {
    try {
      ...
      setData(
        data.filter((existingProduct) => existingProduct._id !== product._id)
      );
    } catch (error) {
      console.error(error);
      alert(error);
    ...
    try {
      ...
      setData(
        data.map((existingProduct) =>
          existingProduct._id === toggledProduct._id
            ? toggledProduct
            : existingProduct
        )
      );
  ...
  return (
    <ProductContext.Provider value={productToEdit}>
      <div>
        <input
          type="checkbox"
