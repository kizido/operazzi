...
const PackageTypeInputField = ({
  ...
  const isFormValid = (): boolean => {
    ...
    const newErrors = {
      ...
      packageLength:
        packageType.packageLength === null || packageType.packageLength <= 0,
      packageWidth:
        packageType.packageWidth === null || packageType.packageWidth <= 0,
      packageHeight:
        packageType.packageHeight === null || packageType.packageHeight <= 0,
      packageWeight:
        packageType.packageWeight === null || packageType.packageWeight <= 0,
    };
  ...
  async function handleAddPackageType() {
    if (isFormValid()) {
      try {
        ...
        productPackageTypeResponse = await ProductsApi.createProductPackageType(
          packageType
        );
  ...
  async function handleEditPackageType() {
    if (isFormValid()) {
      try {
        ...
        productPackageTypeResponse = await ProductsApi.updateProductPackageType(
          packageType,
          packageIdToEdit!
        );
        setAddOptionDialog(false);
    ...
    <div>
      <Form.Group controlId={name + "-input"}>
        ...
        <InputGroup>
          ...
          <IconButton
            onClick={() => setEditOptionMenuDialog(true)}
            className={styles.formButton}
          >
            <IconSettings />
          </IconButton>
      ...
      {addOptionDialog && (
        ...
        >
          <Modal.Header closeButton>
            {packageIdToEdit ? (
              <Modal.Title>Edit {packageType.packageName}</Modal.Title>
            ) : (
          ...
          <Modal.Footer>
            ...
            {packageIdToEdit ? (
              <Button variant="primary" onClick={() => handleEditPackageType()}>
                Apply
              </Button>
            ) : (
              <Button variant="primary" onClick={() => handleAddPackageType()}>
                Add
              </Button>
            )}
          </Modal.Footer>
      ...
      {editOptionMenuDialog && (
        ...
        >
          ...
          <Modal.Body className={modalStyles.dropDownEditModalBody}>
            <Col>
              {packageTypes.map(
                (packageTypeModel: ProductPackageTypeModel, index: number) => {
                  return (
                    <Row key={index} className={modalStyles.editCategoryRow}>
                      ...
                      <Button
                        className={modalStyles.editCategoryRowButton}
                        onClick={() =>
                          deleteProductPackageType(packageTypeModel)
                        }
                      >
                        Delete
                      </Button>
  ...
  async function deleteProductPackageType(
    productPackageType: ProductPackageTypeModel
  ) {
    try {
      ...
      setPackageTypes(
        packageTypes.filter(
          (existingPackageType) =>
            existingPackageType._id !== productPackageType._id
        )
      );
    } catch (error) {
      console.error(error);
