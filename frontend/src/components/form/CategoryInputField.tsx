...
const CategoryInputField = ({
      ...
      {editOptionDialog && (
        ...
        >
          ...
          <Modal.Body className={modalStyles.dropDownEditModalBody}>
            <Col>
              {categories.map(
                (categoryModel: ProductCategoryModel, index: number) => {
                  return (
                    <Row key={index} className={modalStyles.editCategoryRow}>
                      {editingIndex === index ? (
                        <input
                          ...
                          value={newOption.category}
                          onChange={(e) =>
                            setNewOption({ category: e.target.value })
                          }
                          onBlur={() => {
                            valueSubmittedRef.current === true
                              ? handleSave(index)
                      ...
                      <Button
                        ...
                        onClick={() => {
                          ...
                          setNewOption({ category: categoryModel.category });
                        }}
                      >
                        Edit
                      </Button>
  ...
  async function deleteProductCategory(productCategory: ProductCategoryModel) {
    try {
      ...
      setCategories(
        categories.filter(
          (existingCategory) => existingCategory._id !== productCategory._id
        )
      );
    } catch (error) {
      console.error(error);
